...
static inline str_t str_from_size(arena_t* arena, const i64 length);
static inline str_t str_from_cstr(arena_t* arena, const char* cstr);
static inline bool str_valid(const str_t* str);
static inline char* str_to_cstr(arena_t* arena, const str_t* str);
static inline bool str_copy(str_t* destination, const str_t* source);
static inline bool str_eq(const str_t* str1, const str_t* str2);
static inline bool str_eq_cstr(const str_t* str, const char* cstr);
static inline str_t str_slice(const str_t* source, const i64 start, const i64 end);
static inline str_t str_trim(const str_t* str);
static inline bool str_append(str_t* destination, const str_t* source);
static inline bool str_append_cstr(str_t* destination, const char* source);
static inline str_t str_from_append(arena_t* arena, const str_t* destination, const str_t* source);
static inline str_t str_from_append_cstr(arena_t* arena, const str_t* destination, const char* source);
static inline str_t str_from_i64(i64 i);
static inline str_t str_from_i32(i32 i);
static inline str_t str_from_i16(i16 i);
static inline str_t str_from_i8(i8 i);
static inline str_t str_from_u64(i64 i);
static inline str_t str_from_u32(i32 i);
static inline str_t str_from_u16(i16 i);
static inline str_t str_from_u8(i8 i);
static inline i64 str_to_i64(str_t str);
static inline i32 str_to_i32(str_t str);
static inline i16 str_to_i16(str_t str);
static inline i8 str_to_i8(str_t str);
static inline i64 str_to_u64(str_t str);
static inline i32 str_to_u32(str_t str);
static inline i16 str_to_u16(str_t str);
static inline i8 str_to_u8(str_t str);
static inline void str_write(const str_t* str, FILE* file, const bool newline);

// NOT SURE IF I EVEN NEED THIS AT ALL
// bool str_find_next_after(const str_t str, char character, i64* position);

static inline bool str_tokenize(const str_t* str, char separator, i64* context, str_t* token);
...
static inline str_t str_from_cstr(arena_t* arena, const char* cstr) {
    ...
    const i64 source_string_length = (i64)strlen(cstr);
...
static inline bool str_valid(const str_t* str) {
    if (!str || !str->start || str->length > str->capacity) {
        return false;
    }
    return true;
}
...
static inline bool str_append_cstr(str_t* destination, const char* source) {
    ...
    const i64 source_length = (i64)strlen(source);
...
static inline str_t str_from_append(arena_t* arena, const str_t* destination, const str_t* source) {
    ...
    const i64 result_length = destination->length + source->length;
...
static inline str_t str_from_append_cstr(arena_t* arena, const str_t* destination, const char* source) {
    ...
    const i64 source_length = (i64)strlen(source);
    const i64 result_length = destination->length + source_length;
    const str_t result = str_from_size(arena, result_length);
    memmove(result.start, destination->start, destination->length);
...
static inline bool str_tokenize(const str_t* str, const char separator, i64* context, str_t* token) {
    assert(str_valid(str));
    assert(context);
    assert(*context >= -1);
